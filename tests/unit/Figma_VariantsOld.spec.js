...
test('Test parseVarinet', async () => {
  ...
  let parsed = figmaService.parseVariant('mouse=A, size=B')
  expect(parsed.mouse).toBe('A')
  expect(parsed.size).toBe('B')

  parsed = figmaService.parseVariant('mouse=A- size=B')
  expect(parsed.mouse).toBe('A')
  expect(parsed.size).toBe('B')

  parsed = figmaService.parseVariant('type=C')
